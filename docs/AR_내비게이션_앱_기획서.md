...
| 경유지 중복 방지 거리 | 15 m | 기존 경유지 또는 목적지와 너무 가까운 점 추가 방지 |
| 최대 경유지 개수 | 5개 | 지도 가독성과 AR 전환 난이도를 고려한 MVP 상한 |
...
- MVP에서는 2D 보조 화살표를 기본 제공하지 않고 상태 메시지로만 처리한다.
- 2D 보조 화살표는 후속 개선 항목으로 보류한다.
...
| 지도 | 현재 위치 버튼 | 지도를 현재 위치 중심으로 다시 이동 |
...
- 위치 서비스가 켜져 있다.
- 최근의 유효한 현재 위치가 존재한다.
- 현재 위치 정확도가 시작 허용 기준 이내이거나, 기준 초과 시 사용자 확인 다이얼로그에서 명시적으로 계속 진행을 선택했다.
...
- 안내 종료 버튼을 누르면 `현재 안내를 종료할까요?` 확인 다이얼로그를 띄우고, 확인 시 세션을 종료하고 `IDLE`로 초기화한다.
- 목적지 도착 후 확인 버튼을 누르면 진행 세션과 완료 상태를 초기화하고 AR 대기 화면으로 돌아간다. 이전 시작점·경유지·목적지는 자동 복원하지 않는다.
- 시스템 뒤로가기 버튼은 2초 이내 두 번 연속 터치했을 때만 앱을 종료한다. 첫 번째 터치 시에는 `뒤로가기를 한 번 더 누르면 종료됩니다` 안내를 표시한다.
...
- 앱 강제 종료 후 진행 중 세션 복원은 MVP 범위에서 제외한다.
- MVP에서는 앱 재실행 시 항상 새 세션으로 시작하고, 세션 복원은 후속 확장 항목으로 남긴다.
...
| ARCore 미지원 | 지원 기기 필요 안내 | 지도 전용 제한 모드 제공, AR 진입 차단 |
...
| FR-13 | 평면 미인식 시 2D 보조 화살표를 표시할 수 있다. | Later |
| FR-14 | 진행 중인 세션을 앱 재실행 후 복원할 수 있다. | Later |
| FR-15 | 앱은 뒤로가기 버튼을 빠르게 두 번 눌렀을 때만 종료되어야 한다. | Must |
| FR-16 | 지도 화면은 현재 위치 중심으로 다시 이동하는 버튼을 제공해야 한다. | Must |
...
- 현재 위치 버튼을 누르면 사용자가 지도를 이동한 뒤에도 최신 현재 위치 중심으로 다시 이동한다.
...
- 뒤로가기를 한 번 누르면 종료 안내만 표시되고, 2초 이내 한 번 더 눌렀을 때만 앱이 종료된다.
...
1. MVP는 사용자의 지도 터치 좌표를 직선 순서로 안내하는 예제로 확정한다.
2. 진행 중 지도 화면에서는 경로 편집을 허용하지 않는다.
3. AR 평면 미인식 시 2D 보조 화살표는 MVP에 포함하지 않는다.
4. 앱 강제 종료 후 진행 세션 복원은 MVP에 포함하지 않는다.
5. ARCore 미지원 기기에서는 지도 전용 제한 모드를 허용한다.

추가 확정 정책:

6. 지원 화면 방향은 세로 모드 고정으로 한다.
7. 경유지 최소 중복 거리는 15 m, 최대 경유지 개수는 5개로 한다.
8. 시작 시 위치 정확도가 25 m를 초과하면 경고 후 사용자 확인 시에만 진행을 허용한다.
9. 안내 종료는 확인 다이얼로그 후 세션 초기화, 목적지 도착 후에는 확인 시 새 세션 상태로 초기화한다.
10. 시스템 뒤로가기는 2초 이내 두 번 연속 터치 시에만 앱을 종료한다.
11. 지도 화면에는 현재 위치 중심으로 다시 이동하는 버튼을 기본 제공한다.
