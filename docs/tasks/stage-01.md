...
> 우선순위 변경: 이미 검증이 끝난 `S01-T025`~`S01-T027`은 관련 구현 변경이 생기기 전까지 다시 검증 후보로 올리지 않는다. 현재 단계 1은 `S01-T023`, `S01-T024`를 먼저 진행하고, `S01-T007`, `S01-T009`를 그다음 후보로 둔다. 검증·평가성 작업은 필요한 구현이 준비된 뒤에만 다시 활성화한다.
